...
`--codex-timeout-seconds` 与 `--planning-artifact-root`。启动 API 不会调用模型；创建计划或发送讨论
消息才产生一次外部调用。省略 opt-in 或显式 model 会失败关闭。
...
美元/token 上限。真实/付费 smoke test 需要另行明确账户预算和授权，本轮自动验证只使用假 CLI。

## 当前限制与扩展点

- `LocalPlanningAgent` 仍是默认的确定性协议 fixture；Codex adapter 可显式启用，但尚未在本轮
  发起真实/付费 smoke test；
